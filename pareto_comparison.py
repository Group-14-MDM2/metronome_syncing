...
# PD control (from cici's code)

def pd(Kp, Kd, A=0.05, omega=3.13):
    def tau(self, q, dqdt):
...
# FL control (from cici's code)

def fl(Kp_met, Kd_met, Kp_cart=2.0, Kd_cart=0.1, scale=0.1):
    def tau(self, q, dqdt):
        t_vec = np.zeros(self.n)
        q1 = q[0]
        dq1 = dqdt[0]
        ...
        for Kp_cart in fl_Kp_cart_vals:
            for Kd_cart in fl_Kd_cart_vals:
                for scale in fl_scale_vals:
                    tau_func = fl(Kp_met, Kd_met, Kp_cart, Kd_cart, scale)
                    t_sync, E = run_with_energy(params, INITIAL_CONDITIONS.copy(), tau_func, T_END, N_STEPS, COH_THRESH)
...
if pd_synced:
    allpd = [r[3] for r in pd_synced]
    pd_energies_f = [r[4] for r in pd_synced]
else:
    allpd, pd_energies_f = [], [], []

if fl_synced:
    allfl = [r[5] for r in fl_synced]
...
# distance from origin after scaling both axes to [0,1], lower is better, rewards for being simultaneously fast and energy efficiet
def distance(t, e, all_t, all_e):
...
def print_pd_recommendations(pareto):
    ...
    best     = min(pareto, key=lambda x: distance(x[3], x[4], all_t, all_e))
...
def print_fl_recommendations(pareto):
    ...
    best = min(pareto, key=lambda x: distance(x[5], x[6], all_t, all_e))
...
if allpd:
    ax.scatter(pd_energies_f, allpd, color="blue", alpha=0.25,
               s=35, zorder=2, label="_nolegend_")
if allfl:
    ax.scatter(fl_energies_f, allfl, color="red", alpha=0.15,
...
if pd_pareto:
    ...
    best_pd = min(pd_pareto, key=lambda x: distance(x[3], x[4], all_t_pd, all_e_pd))
    ax.scatter(best_pd[4], best_pd[3], color="#8391A6", s=280, marker="*", zorder=5,
               label=f"PD best balanced")

if fl_pareto:
    all_t_fl = [r[5] for r in fl_pareto]
    all_e_fl = [r[6] for r in fl_pareto]
    best_fl = min(fl_pareto, key=lambda x: distance(x[5], x[6], all_t_fl, all_e_fl))
    ax.scatter(best_fl[6], best_fl[5], color="#C6A09D", s=280, marker="*", zorder=5, label=f"FL best balanced point")
